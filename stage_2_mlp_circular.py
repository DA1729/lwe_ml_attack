...
def create_mlp_model(input_shape):
    """Defines an MLP model adapted for the embedded input."""
    model = tf.keras.Sequential([
        tf.keras.layers.Input(shape=input_shape),
        tf.keras.layers.Flatten(), # Flatten the (cos, sin) pairs
...
if __name__ == "__main__":
    ...
    print("--- Stage 2: MLP with Circular Embedding ---")
    ...
    model = create_mlp_model(input_shape=(n + 1, 2))
    ...
    print("Expected outcome: Still severe overfitting, suggesting the issue is architectural.")
